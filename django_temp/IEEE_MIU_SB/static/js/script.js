document.addEventListener("DOMContentLoaded", function () {
  ...
  function hideSpinner() {
    ...
    if (!spinner) {
      console.warn("No #pageSpinner found.");
      return;
    }

    spinner.classList.add("fade-out");

    spinner.addEventListener("animationend", () => spinner.remove(), {
      once: true,
    });

    setTimeout(() => {
      if (document.getElementById("pageSpinner")) {
        document.getElementById("pageSpinner").remove();
      }
    }, 700);
  }
...
new FinisherHeader({
  ...
  colors: {
    background: "#955ec9",
    particles: ["#47f486", "#000000", "#2253e5", "#000000", "#1ad90d"],
  },
  blending: "overlay",
  opacity: {
    center: 0.5,
